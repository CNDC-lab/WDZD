...
def load_data_vgg(dataType):
    if dataType == DataType.WEATHER:
        root_path = '/Users/mikolaj/Desktop/Studia/IV ROK/WDZD/Projekt/dataset'
        # Initialize DataGenerator
        train_gen = IDG(rescale=1./255, validation_split=0.2)
        
        # Load Data
        train_ds = train_gen.flow_from_directory(root_path, target_size=(256,256), class_mode="binary", subset='training', shuffle=True, batch_size=5493)
        test_ds = train_gen.flow_from_directory(root_path, target_size=(256,256), class_mode="binary", subset='validation', shuffle=True, batch_size=1369)
        
        X_train, Y_train = train_ds.next()
        X_test, Y_test = test_ds.next()  
        return np.array(X_train), one_hot(Y_train, 11), np.array(X_test), one_hot(Y_test, 11)
    else:
